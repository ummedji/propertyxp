<?php
/**
 * Codes related to the frontend submission of agents
 */
 ...
 * @param $arguments
 */
function aviators_agent_frontend_submission($arguments) {
    if ($arguments['type'] == 'agent') {
        $form = $arguments['form'];
        $form->addOnValidation('aviators_agent_validate', NULL);
        $form->addOnSuccess('aviators_agent_submit', NULL);

        if(aviators_submission_tos_enabled('agent')) {
...
add_filter('hydra_frontend_submission_form', 'aviators_agent_frontend_submission', 10, 1);
 ...
 * @param $values
 */
function aviators_agent_validate($form, $values) {
 ...
 * @param $values
 */
function aviators_agent_submit($form, $values) {
