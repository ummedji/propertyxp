...
//                         array(&$this, 'settingsPage'));
    ...
    public function saveFormData($cf7) {
        ...
        try {
			...
			if(isset($cf7->posted_data['hidden-author-id'])){
			
				$userid_data = $cf7->posted_data['hidden-author-id'];
				
				$parametrizedQuery = "INSERT INTO `$tableName` (`submit_time`, `form_name`, `field_name`, `field_value`, `field_order`,`user_id`) VALUES (%s, %s, %s, %s, %s,$userid_data)";
				$parametrizedFileQuery = "INSERT INTO `$tableName` (`submit_time`, `form_name`, `field_name`, `field_value`, `field_order`, `file`,`user_id`) VALUES (%s, %s, %s, %s, %s, %s,$userid_data)";
...
//            }
    ...
    public function createAdminMenu() {
        ...
        add_menu_page($displayName,
                        __('Enquiries', 'contact-form-7-to-database-extension'),
...
//        return preg_split('/,|;/', $option, -1, PREG_SPLIT_NO_EMPTY);
    ...
    public function getForms() {
		...
		{
			$query_part .= " where user_id=".$user_id;
		}
		
        $formsFromQuery = $wpdb->get_results("select distinct `form_name` from `$tableName` $query_part  order by `form_name`");
